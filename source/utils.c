...
Debug, will write debug information to sd.... as debug file is open/closed it will be VERY SLOW

*/

#define DEBUG_MAXCACHE 32
static int dbgStarted = 0;
...
void Debug (const char *text, ...)
	{
	static int enabled = 0; // 0 need to check, 1 write header, 2 enabled, -1 disabled
	static char *cache[DEBUG_MAXCACHE];
	int i;
	
	if (!dbgStarted || enabled == -1) 
		return;
	...
	char dbgfile[64];
	sprintf (dbgfile, "%s://postloader.log", vars.mount[DEV_SD]);
	
	if (enabled == 0)
		{
		memset (cache, 0, sizeof(cache));
		if (fsop_FileExist (dbgfile))
			enabled = 1;
		else
			{
			enabled = -1;
