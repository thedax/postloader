...
s32 UMSStorage_GetCapacity(u32 *_sector_size) {
    if (fd > 0) {
        s32 ret;
...
bool umsio_ReadSectors(sec_t sector, sec_t numSectors, u8 *buffer) {
    ...
    while (cnt < numSectors) {
        ...
        /* USB read */
        ret = UMSStorage_ReadSectors(sector + cnt, sectors, &buffer[cnt*512]);
        if (ret < 0)
            return false;
...
bool umsio_WriteSectors(sec_t sector, sec_t numSectors, const u8* buffer) {
    ...
    while (cnt < numSectors) {
        ...
        /* USB write */
        ret = UMSStorage_WriteSectors(sector + cnt, sectors, &buffer[cnt * 512]);
        if (ret < 0)
            return false;
